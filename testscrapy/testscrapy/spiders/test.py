...
class PostsSpider(scrapy.Spider):
  ...
  # current_brand = ''
  start_urls = [
    # 'https://item.rakuten.co.jp/e-kurashi/c/0000000114/'
    # 'https://www.rakuten.co.jp/category/100890/',
    'https://www.rakuten.co.jp/category/100012/'
    # 'https://search.rakuten.co.jp/search/mall/-/100012/?max=12&min=1'
  ...
  def start_requests(self):
    for url in self.start_urls:
      ...
      for i in range(1,1000000,2):
        search_url = f'{url}/?max={i}&min={i+1}'
        yield scrapy.Request(search_url, callback=self.parse_page)

  def parse_page(self, res):
    self.no_of_pages -= 1

    # link_products = res.xpath("//a[@class='category_itemnamelink']").xpath("@href").getall()
    link_products = res.xpath("//div[@class='dui-cards searchresultitems']//div//div[2]//h2[1]//a[1]/@href").getall()
